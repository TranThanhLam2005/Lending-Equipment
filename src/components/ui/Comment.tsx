...
const Comment = memo(({ message, isMine }) => {
    const formatCreateAt = format(parseISO(message.createAt), 'EEEE, HH:mm:ss');
    return (
        <div>
            {!isMine && <div className="flex items-center gap-3">
                <img
            ...
            <div className={`flex items-center ${isMine ? 'justify-end' : 'justify-start'}`}>
                <span className={` text-sm p-2 rounded-full ${isMine ? 'bg-red-500 text-white' : 'bg-gray-200 mt-2'} `}>
                    {message.content}
                </span>
            </div>

            {isMine && <div className="text-sm text-gray-500 text-right mt-1">
                {formatCreateAt}
            </div>}
        </div>
    );
